...
class ShopifyController extends Controller
{
    public function index(){
        ...
        if(request()->ajax()) {
            return datatables()->of(Shopify::select('*')->where('shop_id',$users->shop_id))
            ->addColumn('action', 'shopify.action')
            ->rawColumns(['action'])
            ->addIndexColumn()
            ->make(true);
...
// dd($product);
                $exist_data = Shopify::where('product_id',$product['id'])->where('shop_id',$shop_id)->first();
                if($exist_data != null){
    ...
    public function update_qty_up() {
        ...
        $mypos_data_shopify = MyPos::where('shop_id',$users->shop_id)->where('code','!=',Null)->get();
