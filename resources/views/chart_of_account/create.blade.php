              ...
              <div class="row widget-content widget-content-area">
                                  ...
                                  <li class="text-warning">@if($user_role == 3)<a href="javascript:void(0)" data-toggle="tooltip" onClick="editFunc({{ $coa2->id }})" data-original-title="Edit" class="edit text-info">@endif {{$coa2->name ?? ''}} @if($user_role == 3)</a>@endif</li>
                                    @if($coa2->child)
                                        <ul>
                                          ...
                                          <li class="text-info">@if($user_role == 3)<a href="javascript:void(0)" data-toggle="tooltip" onClick="editFunc({{ $coa2->id }})" data-original-title="Edit" class="edit text-info">@endif {{$coa3->name ?? ''}} @if($user_role == 3)</a>@endif</li>
                                          @if($coa3->child)
                                          <ul>
                                            @foreach($coa3->child as $coa4)
                                            <!-- <li class="text-danger">{{$coa4->name ?? ''}}</li> -->
                                            <li class="text-danger">@if($user_role == 3)<a href="javascript:void(0)" data-toggle="tooltip" onClick="editFunc({{ $coa2->id }})" data-original-title="Edit" class="edit text-info">@endif {{$coa4->name ?? ''}} @if($user_role == 3)</a>@endif</li>
                                                  @endforeach
                                                </ul>
                                            @endif
                                          @endforeach
                  ...
                  <form class="g-3" action="{{route('chart_of_account.store')}}" method="POST">
                      ...
                      <div class="form-group mb-3">
                          ...
                          <input type="text" class="form-control" id="name" name="name" placeholder="Account Head">
                      </div>
                      <div class="form-group mb-3">
